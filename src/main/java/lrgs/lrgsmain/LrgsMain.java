...
import lrgs.archive.PublishSNS;
...
public class LrgsMain
	...
	public void run() {
		...
		if (System.getProperty("os.name").toLowerCase().startsWith("linux")) {
			...
			try {
				SignalTrapper.setSignalHandler(SIGHUP, this);
			} catch (Throwable ex) {
	...
	private boolean initLRGS() {
		...
		if (cfg.getMiscBooleanProperty("noaaport.enable", false)
			...
			Logger.instance().debug1("NOAAPORT _not_ enabled.");
		...
		if (cfg.networkDcpEnable) {
			...
			statusProvider.getStatusSnapshot().networkDcpStatusList = networkDcpRecv.getStatusList();
		}
		...
		// ddsRecv2.setLastMsgRecvTime(statusProvider.getLastReceiveTime()); // for
		// secondary group
		// }
		...
		HashMap<String, LoadableLrgsInputInterface> nm2if = new HashMap<String, LoadableLrgsInputInterface>();
		...
		for (Enumeration<Object> kenum = props.keys(); kenum.hasMoreElements();) {
			...
			if (TextUtil.startsWithIgnoreCase(key, "LrgsInput.")
				...
				try {
					...
					LoadableLrgsInputInterface input = (LoadableLrgsInputInterface) inputClass.newInstance();
	...
	private boolean initDdsServer() {
		...
		try {
			InetAddress ia = (cfg.ddsBindAddr != null && cfg.ddsBindAddr.length() > 0)
					? InetAddress.getByName(cfg.ddsBindAddr)
					: null;
	...
	public static void main(String args[])
		...
		/**
		 * Using lock files as an IPC mechanism (for status GUI) is unreliable in
		 * windoze.
